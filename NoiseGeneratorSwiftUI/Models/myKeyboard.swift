...
@IBDesignable open class myKeyboardView: UIView, AKMIDIListener {
    //swiftlint:disable
    ...
    //my functions
    func notesFromPoints(_ points: [CGPoint]) -> [MIDINoteNumber] {
        var notes = [MIDINoteNumber]()
        for point in points {
            if let note = noteFromTouchLocation(point) {
                notes.append(note)
            }
        }
        return notes
    ...
    /// Handle new touches
    open func touchesBegan(_ points: [CGPoint]) {
        let notes = notesFromPoints(points)
        for note in notes {
            pressAdded(note)
        }
        setNeedsDisplay()
    ...
    open func touchesEnded(_ points: [CGPoint]) {
        for point in points {
            if let note = noteFromTouchLocation(point) {
    ...
    open func touchesMoved(_ points: [CGPoint]) {
        let notes = notesFromPoints(points)
        for note in notes {
            pressAdded(note)
        }
        ...
        /*
        for point in points {
            let key = noteFromTouchLocation(point)
            pressAdded(key)
            setNeedsDisplay()
            /*
            if let key = noteFromTouchLocation(point),
    ...
    private func pressRemoved(_ note: MIDINoteNumber) {
        guard onKeys.contains(note) else {
            return
        }
        onKeys.remove(note)
